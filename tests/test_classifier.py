...
def test_classify_incident(mocker):
    ...
    mocker.patch("customer_support_rag.classifier.prompt_tester", return_value=mock_response)
    result = classify_ticket(mocker.MagicMock(), "Our system is down and we can't access anything!")
...
def test_classify_invalid_json(mocker):
    mocker.patch("customer_support_rag.classifier.prompt_tester", return_value="Not a JSON response")
